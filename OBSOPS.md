...
```sh
du -sh ~/.ollama ~/.ollama/models .cursor/pseudini 2>/dev/null
python3 -m json.tool .cursor/pseudini/cache-v1/manifest.json
lsof -nP -iTCP:11434
npm test
npm run benchmark
```
...
```sh
PSEUDINI_BENCH_RUNS=5 \
PSEUDINI_BENCH_WARMUPS=1 \
PSEUDINI_BENCH_MODELS=qwen2.5-coder:3b \
PSEUDINI_BENCH_CASES=small-general \
PSEUDINI_BENCH_OUTPUT=local-check.json \
npm run benchmark
```
...
3. Pseudini parses the `pseudini:` instructions and selects the deterministic, local-model, or
   provider route.
...
pseudocode. They are not the word count of the `pseudini:` comment. The output count includes the JSON
...
```sh
du -sh ~/.ollama ~/.ollama/models .cursor/pseudini pseudini-0.1.0.vsix 2>/dev/null
df -h .
ollama list
```
...
```sh
python3 -m json.tool .cursor/pseudini/cache-v1/manifest.json
ls -lt .cursor/pseudini/cache-v1/files
```
...
```sh
git check-ignore -v .cursor/pseudini/cache-v1/manifest.json
```
...
- Delete `.cursor/pseudini/` to force disposable cache regeneration.
...
Use `PSEUDINI_BENCH_DEBUG=1` only with non-sensitive fixtures. It prints generated model output.

Run the whole-file suite separately:

```sh
PSEUDINI_BENCH_RUNS=3 \
PSEUDINI_BENCH_MODELS=qwen2.5-coder:3b \
...
- Inspect the active file and `.cursor/pseudini` cache facts.
...
- Inspect `.cursor/pseudini/cache-v1/manifest.json`.
- Delete `.cursor/pseudini/` and reopen the file.
